...
1. **User must have email or phone**:
   - Enforced at application layer (TypeORM @BeforeInsert)

2. **Record time cannot be in future**:
   - Enforced at application layer

3. **Details must match record_type schema**:
   - Validated using class-validator with discriminated unions

### Cascading Behavior

- **User deletion**: Cascade delete all associated BabyRecords
- **Record deletion**: Soft delete not implemented (hard delete only)

---

## Performance Considerations

1. **Pagination**: Always use LIMIT/OFFSET for list queries
2. **Indexing**: Composite index on (user_id, record_time) for timeline
3. **JSON Queries**: Use SQLite JSON functions (JSON_EXTRACT) for statistics
4. **Connection Pooling**: Use better-sqlite3 for synchronous, pooled connections
